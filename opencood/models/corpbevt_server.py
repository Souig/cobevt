...
from opencood.comm.server import Server
...
class CorpBEVTServer(nn.Module):
    def __init__(self, config):
        super(CorpBEVTServer, self).__init__()
    ...
    def forward(self, batch_dict):
        ...
        #each agents performs Resnet separately
        result = [self.encoder(x[0,:,:,:,:,:].unsqueeze(0))]
        server = Server(self.max_cav)
        result = result + server.messages
        #stack the features of different agents
        x_result = [torch.cat([result[j][i] for j in range(b)],dim=0) for i in range(len(result[0])) ]
        x = x_result
