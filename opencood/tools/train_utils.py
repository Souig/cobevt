...
def load_saved_train_model(saved_path, model):
    """
...
def load_saved_deploy_model(saved_path, model, num):
    """
    Load saved model if exiseted
    ...
    initial_epoch = findLastCheckpoint(saved_path) - num*5
    if initial_epoch > 0:
        print('resuming by loading epoch %d' % initial_epoch)
        checkpoint = torch.load(
            os.path.join(saved_path,
                         'net_deploy_epoch%d.pth' % initial_epoch),
            map_location='cpu')
        model.load_state_dict(checkpoint, strict=False)
