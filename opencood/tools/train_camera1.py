...
def main():
    ...
    hypes['model']['args']['encoder']['deploy'] = False
    ...
    if opt.model_dir:
        ...
        init_epoch, model = train_utils.load_saved_train_model(saved_path,
                                                         model)
    ...
    file_str = "epoch          " + 'Validate_Loss          ' + 'Dynamic_Iou          ' \
                 + 'Road_IoU          ' + 'Lane_IoU          '
    fp.write(file_str)
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    ...
                    if len(iou_dynamic) >= 2:
                        dynamic_ave_iou.append(iou_dynamic[1])
                    else:
                        dynamic_ave_iou.append(0)
            ...
            print('At epoch %d, the validation loss is %f,'
                  ...
                  % (epoch, valid_ave_loss, dynamic_ave_iou))

            writer.add_scalar('Validate_Loss', valid_ave_loss, epoch)
            writer.add_scalar('Dynamic_Iou', dynamic_ave_iou, epoch)
            if os.path.exists(filename):
                fp = open(filename, mode="r+", encoding="utf-8")
                file_str = str(epoch) + '          '+ str(valid_ave_loss) + '     ' + str(dynamic_ave_iou)
        ...
        if epoch % hypes['train_params']['save_freq'] == 0:
            ...
            # copy and convert to deploy mode
            model_deploy = copy.deepcopy(model_without_ddp)
            for module_deploy in model_deploy.modules():
                if hasattr(module_deploy, 'switch_to_deploy'):
                    module_deploy.switch_to_deploy()
            torch.save(model_deploy.state_dict(),
                       os.path.join(saved_path,
                                    'net_deploy_epoch%d.pth' % (epoch + 1)))
