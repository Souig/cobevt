...
class BaseCameraDataset(basedataset.BaseDataset):
    ...
    def visualize_agent_bbx(self, data_sample, agent, draw_3d=True,
        ...
        draw_image = self.visualize_agent_camera_bbx(agent_sample,'camera0',draw_3d,color,thickness)
        draw_image_list.append(draw_image)
...
if __name__ == '__main__':
    params = load_yaml('/home/ubuntu/cobevt/opencood/hypes_yaml/opcamera/base_camera_test.yaml')

    opencda_dataset = BaseCameraDataset(params, train=True, visualize=True)
    root = '/home/ubuntu/cobevt/draw_bbx/'
    for i in range(len(opencda_dataset)):
        draw_root = root + '0'*(6-len(str(i))) + str(i) + '_draw.png'
        data_example = opencda_dataset.get_sample(0,i)
        draw_image_list, cav_id_list =\
            opencda_dataset.visualize_all_agents_bbx(data_example)

        camera_utils.plot_all_agents(draw_image_list, cav_id_list, draw_root)
